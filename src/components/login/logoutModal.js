    ...
    <Dialog open={open} handler={handler} className={'bg-[#121212] rounded-[28px] p-[30px]'} size={'xs'}>
      <DialogBody className={'p-0 w-full h-full'}>
        <div className={'flex flex-col gap-[16px] font-medium'}>
          ...
          <div className={'desktop flex flex-col py-[10px] px-10'}>
          ...
          <div className={'mobile flex gap-2'}>
            <Button tabIndex={-1} className={'flex-1 text-[16px] text-[#9C9C9C] bg-transparent'} size={'sm'} onClick={onCancel} variant={'outlined'}>取消</Button>
            <Button tabIndex={-1} className={'flex-1 text-[16px] text-shayuBlue'} color={'blue'} size={'sm'} onClick={onConfirm} variant={'outlined'}>确定</Button>
          </div>
        </div>
      </DialogBody>
    </Dialog>
