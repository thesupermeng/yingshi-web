  ...
  return (
    ...
    >
      ...
      <div className={'flex-1 w-full absolute'}>
        ...
        {isBest &&
          <div
            className={'flex w-fit items-center justify-center rounded-tl-[8px] rounded-br-[8px] bg-[#FA3E3E] px-[9px] py-[2px] top-0 left-0'}>
            <span className={'text-white text-[11px] font-semibold'}>最多人选择</span>
          </div>
        }
        <div
      ...
      <div
        className={`flex-1 w-full flex flex-col items-center justify-center gap-0.25 ${style.product_card_background_color} ${isSelected ? style.selected : ''}`}>
        <span className={'text-[16px] text-white font-semibold'}>{productInfo.product_name}</span>
        <span className={'text-[16px] text-[#F4DBBA] font-bold'}>{productInfo.currency.currency_symbol}{productInfo.product_price}</span>
        {productInfo.product_fake_price && //has fake price
          ...
          <span className={'text-[14px] text-[#9C9C9C] font-medium'}><s>{productInfo.currency.currency_symbol}{productInfo.product_fake_price}</s></span>
        }
      </div>
      {isSelected && (
        <div className={'h-[35px] bg-[#F9EBDB] flex items-center justify-center'}>
          <span className={'text-[14px] text-black font-semibold'}>{productInfo.product_desc}</span>
        </div>
      )}
      {!isSelected && (
        <div className={'h-[37px] bg-[#393939] flex items-center justify-center'}>
          <span className={'text-[14px] text-white font-semibold'}>{productInfo.product_desc}</span>
        </div>
      )}
    </div>
  )
