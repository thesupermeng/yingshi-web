...
const DefaultStream = { fStream: null, fStreamIdx: null };

export const useFocusStream = (prop) => {
  const { autoFocus } = prop || { autoFocus: false };
  const { streams } = useStreams();
  const dispatch = useDispatch();
  const focusStreamId = useSelector((s) => s.streams.focusStreamId);
  const [focusStream, setFocusStream] = useState(DefaultStream);
  const pathname = usePathname();
  const isSportsPage = pathname.startsWith('/sports');

  useEffect(() => {
    if (streams.length > 0 && !isSportsPage) {
      if (focusStreamId) {
        const fStream = streams?.find((a) => a.streamer_id == focusStreamId);
        const fStreamIdx = streams?.findIndex(
          (a) => a.streamer_id == focusStreamId
        );
        if (fStreamIdx !== -1) {
          setFocusStream({ fStream, fStreamIdx });
          dispatch(setMatchId(fStream?.match?.match_id));
        } else {
          setFocusStream({ fStream: streams[0], fStreamIdx: 0 });
          dispatch(setMatchId(streams[0]?.match?.match_id));
        }
      } else if (autoFocus && !focusStreamId) {
        dispatch(setFocusStreamId(streams[0].streamer_id));
      }
    }
  }, [streams, focusStreamId, autoFocus]);

  return {
    focusStream: focusStream.fStream,
    focusStreamIndex: focusStream.fStreamIdx,
  };
};
